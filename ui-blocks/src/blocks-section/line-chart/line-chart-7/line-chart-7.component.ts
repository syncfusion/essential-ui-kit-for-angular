...
export class LineChart7Component implements OnInit, OnDestroy {
    ...
    public tooltipRender(args: any): void {
        args.text = `Date : ${this.chartData[args.data.pointIndex].date.toLocaleDateString()}<br>Close : $${this.chartData[args.data.pointIndex].close}<br>Open : $${this.chartData[args.data.pointIndex].open}<br>High : $${this.chartData[args.data.pointIndex].high}<br>Low : $${this.chartData[args.data.pointIndex].low}<br>Volume : $${this.chartData[args.data.pointIndex].volume.toLocaleString()}`
    };
    ...
    public resize(): void {
        ...
        if (this.chartDropdown.element.classList.contains('e-active')) {
            this.chartDropdown.toggle();
        }
    }
